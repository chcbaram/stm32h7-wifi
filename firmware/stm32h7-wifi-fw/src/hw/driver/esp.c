...
static uint8_t uart_ch = HW_ESP_UART_CH;
...
void cliEsp(cli_args_t *args)
{
  ...
  if (args->argc == 2 && args->isStr(0, "open"))
  {
    ...
    if (espOpen(uart_ch, baud))
    {
      cliPrintf("espOpen %d, OK\n", baud);
    }
    else
    {
  ...
  if (args->argc == 3 && args->isStr(0, "at"))
  {
    ...
    if (espCmd(args->getStr(1), timeout) == true)
    {
      ...
      cliPrintf("ovr_line : %d\n", esp_resp.ovr_line);
      cliPrintf("ovr_bytes: %d\n", esp_resp.ovr_bytes);
